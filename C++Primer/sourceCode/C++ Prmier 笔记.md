...
- **reinterpret_cast**：通常为运算对象的位模式提供低层次上的重新解释。第五章 语句
  ...
  - **返回指向函数的指针**：1.类型别名；2.尾置返回类型。第七章 类 （Class）
    ...
    - 初始化：
      ...
      - 如果一定要在类内部定义，则要求必须是字面值常量类型的`constexpr`。第八章 IO库
        ...
        上表中`sstream`是头文件`sstream`中任意一个类型。`s`是一个`string`。第九章 顺序容器
        ...
        - `queue`可以在`list`或`vector`之上实现，`priority_queue`也可以用`deque`实现。第十章 泛型算法
